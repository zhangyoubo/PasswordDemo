...
- (void)initPwdTextField
{
    ...
    for (int i = 0; i < kDotCount; i++) {
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.textField.frame) + i * width + 10, self.frame.size.height - 2, width - 20, 2)];
    ...
    //生成中间的点
    for (int i = 0; i < kDotCount; i++) {
        UILabel *dotView = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.textField.frame) + i * width, CGRectGetMinY(self.textField.frame), width, 60)];
...
//        NSLog(@"%@", textField.text);
        if ((NSInteger)textField.text.length > 0) {
            //((UILabel *)[self.dotArray objectAtIndex:textField.text.length]).hidden = NO;
            ((UILabel *)[self.dotArray objectAtIndex:textField.text.length-1]).text = @"";
        ...
        if ((NSInteger)textField.text.length >= 0) {
            ((UILabel *)[self.dotArray objectAtIndex:textField.text.length]).hidden = NO;
            ((UILabel *)[self.dotArray objectAtIndex:textField.text.length]).text = string;
        }
